...
def _is_valid_openapi_spec(open_api_dict: dict, json_file_path: str):

    required_general_keys = ["info", "host", "schemes", "basePath", "paths"]

    all_general_keys = all(key in open_api_dict for key in required_general_keys)
...
def openapi_to_yaml(json_file_path: str):
    ...
    base_yaml = {"api": {"endpoints": [{"name": None, "path": None, "requests": []}]}}
    ...
    with open("./api.yaml", "w") as file:
        file.write(
            yaml.dump(base_yaml, default_flow_style=False, sort_keys=False, indent=4)
        )
