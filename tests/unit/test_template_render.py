...
class TestRender:
    ...
    @mark.it("should not render unsupported content type")
    def test_should_not_render_unsupported_content_type(self, mocked__request):
        request = mocked__request()
        request.headers = {
            "Content-Length": "0",
        ...
        request.body = b""
        assert (
            "Cannot render. Unsupported content type: application/octet-stream."
            == render_body(request)
        )
